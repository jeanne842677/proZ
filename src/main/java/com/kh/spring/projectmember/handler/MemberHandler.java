...
import com.kh.spring.projectmember.controller.ProjectMemberController;
...
public class MemberHandler implements ChannelInterceptor {
	...
	public Message preSend(Message<?> message, MessageChannel channel) {
		...
		if (command.compareTo(StompCommand.SUBSCRIBE) == 0) {
			...
			/*
			 * Member member = new Member(); member.setUserIdx("test");
			 * 
			 * MessageHeaders mh = new MessageHeaders(Map.of("content-type",
			 * StompCommand.SEND, "simpDestination", "/app/remove/999998", "simpSessionId",
			 * accessor.getSessionId())); Message<Member> msg =
			 * MessageBuilder.createMessage(member, mh);
			 * 
			 * System.out.println("구독 주소 : " + destination); System.out.println(message);
			 * 
			 * channel.send(msg);
			 */

			/*
			 * String[] url = destination.split("/"); ProjectMemberSession pmm = new
			 * ProjectMemberSession("그냥", "해봄"); if (url[0].equals("online")) { String
			 * projectIdx = url[url.length - 1]; String session = accessor.getSessionId();
			 * 
